...
import { CreateAnnouncementRequest, UpdateAnnouncementRequest } from "../services/announcements";
...
export const CreateUpdateAnnouncement = ({
    ...
    const handleOnOk = async () => {
        if (mode === Mode.Create) {
            const announcementRequest: CreateAnnouncementRequest = { title, description, quantityAvailable, pricePerDay };
            handleCreate(announcementRequest);
        } else if (mode === Mode.Edit) {
            const announcementRequest: UpdateAnnouncementRequest = { id: values.id, title, description, quantityAvailable, pricePerDay };
    ...
    return (
        <Modal
            title={mode === Mode.Create ? "Добавить объявление" : "Редактировать объявление"}
            open={isModalOpen}
            cancelText={"Отмена"}
            onOk={handleOnOk}
