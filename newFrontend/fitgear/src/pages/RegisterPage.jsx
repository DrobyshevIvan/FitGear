    ...
    return (
        <>
            ...
            <div className="flex justify-center items-center min-h-screen">
                <form className="flex flex-col items-center w-[25vw] min-w-[400px] h-[680px]  md:mt-[-100px] border border-gray-300 shadow-xl/10 rounded-md">
                    <h1 className="text-5xl my-6">Fitgear</h1>
                    <h3 className="text-3xl mt-2 mb-8 border-b-1 w-95 py-3">Create an account</h3>
                    <div className="w-90">
                        <input type="text" 
                    ...
                    <div className="mt-6">
                        <div className="relative w-90">
                            ...
                            className="py-3 text-md ps-4 pe-10 block w-full border border-gray-300 
                            rounded-lg focus:border-blue-500 focus:ring-blue-500" 
                            placeholder="Enter password" required/>
                            <button
                                type="button"
                                onClick={() => setHidden(prev => !prev)}
                                className="absolute inset-y-0 right-0 flex items-center px-3 cursor-pointer text-gray-400 rounded-e-md focus:outline-none"
                        ...
                        <div className="relative w-90 mt-5">
                            <input type={isHiddenConfirm  ? "password" : "text"} onChange={e => setConfirmedPassword(e.target.value)}
                                className="py-3 text-md ps-4 pe-10 block w-full border border-gray-300 
                            rounded-lg focus:border-blue-500 focus:ring-blue-500" placeholder="Confirm password" required />
                            <button
                                type="button"
                                onClick={() => setHiddenConfirm(prev => !prev)}
                                className="absolute inset-y-0 right-0 flex items-center px-3 cursor-pointer text-gray-400 rounded-e-md focus:outline-none"
                            >
                                {isHiddenConfirm ? <EyeOff size={20} /> : <Eye size={20} />}
                            </button>
                        </div>
                    </div>
                    {error && <p className="text-red-600 my-2">{error}</p>}
                    <button onClick={handleSignUp} className="bg-blue-700 text-lg text-white w-60 mt-6 py-3 border border-blue-700 cursor-pointer hover:bg-white hover:text-black select-none">
                        Sign up
                    </button>
